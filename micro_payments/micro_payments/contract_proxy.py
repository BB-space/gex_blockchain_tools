...
class ChannelContractProxy(ContractProxy):
    ...
    def get_maintainer_registered_logs(self, from_block=0, to_block='latest', filters=None):
        return self.get_logs('MaintainerRegistered', from_block, to_block, filters)

    def get_channel_topic_created_logs(self, from_block=0, to_block='latest', filters=None):
        return self.get_logs('ChannelTopicCreated', from_block, to_block, filters)

    def get_collateral_payed_logs(self, from_block=0, to_block='latest', filters=None):
        return self.get_logs('CollateralPayed', from_block, to_block, filters)

    def get_closing_balances_changed_logs(self, from_block=0, to_block='latest', filters=None):
        return self.get_logs('ClosingBalancesChanged', from_block, to_block, filters)

    def get_channel_created_event_blocking(
            self, sender, from_block=0, to_block='latest',
            wait=DEFAULT_RETRY_INTERVAL, timeout=DEFAULT_TIMEOUT
    ):
        filters = {
           '_sender': sender
    ...
    def get_channel_topped_up_event_blocking(
        ...
        return self.get_event_blocking(
            'ChannelToppedUp', from_block, to_block, filters, None, wait, timeout
        )
    ...
    def get_channel_topic_created_event_blocking(
            self, sender, opening_block,
            from_block=0, to_block='latest', wait=DEFAULT_RETRY_INTERVAL, timeout=DEFAULT_TIMEOUT
    ):
        filters = {
        ...
        return self.get_event_blocking(
            'ChannelTopicCreated', from_block, to_block, filters, None, wait, timeout
        )
    ...
    def get_maintainer_registered_event_blocking(
        ...
        return self.get_event_blocking(
            'MaintainerRegistered', from_block, to_block, filters, None, wait, timeout
        )

    def get_cheating_reported_event_blocking(
            self, sender, opening_block,
            from_block=0, to_block='latest', wait=DEFAULT_RETRY_INTERVAL, timeout=DEFAULT_TIMEOUT
    ):
        filters = {
        ...
        return self.get_event_blocking(
            'CheatingReported', from_block, to_block, filters, None, wait, timeout
        )
    ...
    def get_settle_timeout(self, sender, open_block_number):
        try:
            ...
            # attempt to get info on a closing request that doesn't exist
            return None
        return closing_request_info[0]
