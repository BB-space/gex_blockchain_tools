...
contract GexBotNew is StandardToken, Ownable{
    struct Cell {
        bool active;
        uint88 amountEth;
        uint88 amountGex;
    ...
    uint88 public gexInitialReserve;
    uint88 public ethInitialReserve;
    uint40 public numberOfAddresses;
    uint88 public currentGex;
    uint88 public currentEth;
    uint88 public previousGex;
    uint88 public previousEth;
    uint40 public previousTime;
    uint24 constant public DAY = 300;

    mapping (address => Cell) public applications;
    mapping (uint40 => address) public addresses;
    ...
    function putEth(bytes _data) public payable {
        ...
        uint88 debt = uint88(-applications[msg.sender].sendEth);
        if (debt > 0) {
            if (amountEth >= debt) {
                amountEth -= debt;
        ...
        if (amountEth > 0) {
            ...
            amountEth -= fee(amountEth, true); //up to 30 000 gas
    ...
    function timeToChange() public { // about 23 000 gas
        require(previousTime + DAY < block.timestamp);
    ...
    //Private functions
    function putGex(bytes _data, uint88 _amount) public {
        require(previousTime + DAY > block.timestamp);
        if (_data.length != 0) {
            bytes26 flag;
            bytes26 addReserveKey = bytes26("addgalacticexchangereserve");
        ...
        uint88 debt = uint88(-applications[msg.sender].sendGex);
        if (debt > 0) {
            if (_amount >= debt) {
                amountGex -= debt;
                applications[msg.sender].sendGex = 0;
                ...
                amountGex = 0;
                applications[msg.sender].sendGex += int88(_amount);
            }
            Message("Debt paid in gex", msg.sender, _amount - amountGex);
        }
        if (amountGex > 0) {
            uint40 index = numberOfAddresses;
            if (!applications[msg.sender].active) {
            ...
            amountGex -= fee(amountGex, false);  //up to 30 000 gas
            applications[msg.sender].amountGex += uint88(amountGex);
            currentGex += uint88(amountGex);
            uint88 amount = uint88((uint256((amountGex * 80) / 100) *
                uint256(previousEth + currentEth)) /
                uint256(previousGex + currentGex));
            applications[msg.sender].loanEth += uint88(amount);
            transferEth(
                msg.sender,
                amount,
        ...
        returns (uint88)
    {
        ...
        if (_ethCur) {
            ...
            stateReserve = uint88(balanceOf(address(this)));
        } else {
        ...
        public
    {
        ...
        require(transfer(_sender, _amount));
    }
